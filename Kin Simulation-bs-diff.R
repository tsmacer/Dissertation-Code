...
# # Function to calculate population SD
...
# }
# 
# # Original sample variation from K&K
# # population_sd(c(12, 15, 18))
# 
# test <- c(40, 20)
# 
# # Function to calculate sample size variation coefficient
# n_variation <- function(sd, mean_n) {
#   sd/mean(mean_n)
# }
# 
# # 0.16 as reported in the paper
# # n_variation(population_sd(c(12, 15, 18)), c(12, 15, 18))
# 
# n_variation(population_sd(test), test)
# 
# #40, 20
# #18, 42
# #25, 35
...
for (i5 in 1:3) {
  ...
  for (i4 in seq_along(n_loop)) {
    ...
    for (i in seq_len(NSIM)) {
      ...
      if (n == 6) {
        dat$group <- factor(rep(c("A", "B"), c(2, 4)))
      } else if (n == 10) {
        dat$group <- factor(rep(c("A", "B"), c(3, 7)))
      } else if (n == 20) {
        dat$group <- factor(rep(c("A", "B"), c(13, 7)))
      } else if (n == 30) {
        dat$group <- factor(rep(c("A", "B"), c(10, 20)))
      } else if (n == 60) {
        dat$group <- factor(rep(c("A", "B"), c(40, 20)))
      }
...
#save(all_multi, all_uni, all_anova, matrix_list, file = "results10_unequal.rda")

#Load in one of the RDA files first so the list can calculate length of dataframes

file_list <- c("results1_unequal.rda", "results2_unequal.rda", "results3_unequal.rda", 
               "results4_unequal.rda", "results5_unequal.rda", "results6_unequal.rda", 
               "results7_unequal.rda", "results8_unequal.rda", "results9_unequal.rda", 
               "results10_unequal.rda")
...
#save(combined_anova, combined_multi, combined_uni, file = "all_results_bs_diff.rda")
...
unequal_plot <- combined_out %>%
  ...
  ylim(0.02, 0.1) +
  ggtitle("Between-Subjects Unequal Groups Design") +
...
# Combined Plot
bs_diff_plot_only_sig <- combined_out_only_sig %>%
  ggplot(aes(x = n, group = method, colour = method)) +
  geom_hline(yintercept = 0.05) +
  geom_pointrange(aes(y = prop_signif, ymin = lower, ymax = upper),
                  position = position_dodge(2)) +
  facet_grid(rows = vars(epsilon), cols = vars(correction)) + 
  ylim(0, 0.1) +
  ggtitle("Between-Subjects Unequal Groups") +
...
bs_diff_plot_anova <- combined_out_anova %>%
  ggplot(aes(x = n)) +
  geom_hline(yintercept = 0.05) +
  geom_pointrange(aes(y = prop_signif, ymin = lower, ymax = upper),
                  position = position_dodge(2)) +
